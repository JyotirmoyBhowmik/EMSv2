Describe 'EMS.Jwt' {
    ...
    It 'round-trips a valid token' {
        $t = New-EMSJwt -Subject 'alice' -Role 'admin' -Groups @('g1','g2') -Secret $global:secret
        $c = ConvertFrom-EMSJwt -Token $t -Secret $global:secret
        $c.sub  | Should -Be 'alice'
        $c.role | Should -Be 'admin'
    }

    It 'rejects tampered payload' {
        $t = New-EMSJwt -Subject 'alice' -Role 'user' -Secret $global:secret
        $parts = $t -split '\.'
        $bad = "$($parts[0]).eyJzdWIiOiJhbGljZSIsInJvbGUiOiJhZG1pbiJ9.$($parts[2])"
        ConvertFrom-EMSJwt -Token $bad -Secret $global:secret | Should -BeNullOrEmpty
    }

    It 'rejects wrong secret' {
        $t = New-EMSJwt -Subject 'alice' -Role 'admin' -Secret $global:secret
        ConvertFrom-EMSJwt -Token $t -Secret 'different-secret' | Should -BeNullOrEmpty
    }

    It 'rejects expired token' {
        $t = New-EMSJwt -Subject 'alice' -Role 'admin' -Secret $global:secret -ExpiresIn -40
        ConvertFrom-EMSJwt -Token $t -Secret $global:secret | Should -BeNullOrEmpty
    }
    ...
    It 'rejects alg=none / wrong alg' {
        ...
        ConvertFrom-EMSJwt -Token "$h.$p." -Secret $global:secret | Should -BeNullOrEmpty
    }
}
