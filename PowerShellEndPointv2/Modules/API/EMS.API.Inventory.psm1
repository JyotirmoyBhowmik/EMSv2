...
function Invoke-ComputerRoutes {
    param(
...
"@ -Parameters @{ computerName = $computerName } 
    ...
    if ($Method -eq 'GET' -and $Path -eq '/computers') {
        if (-not (Test-ViewerAccessRequirement -Request $Request -Response $Response -Config $Config)) { return $true }
    ...
    if ($Method -eq 'POST' -and $Path -eq '/computers') {
...
"@ -Parameters @{ scanId = $resultId } | Select-Object -First 1
    ...
    if ($Method -eq 'GET' -and $Path -eq '/results') {
        if (-not (Test-ViewerAccessRequirement -Request $Request -Response $Response -Config $Config)) { return $true }
...
function Invoke-DashboardRoutes {
    param(
        [System.Net.HttpListenerRequest]$Request,
        [System.Net.HttpListenerResponse]$Response,
        [string]$Method,
    ...
    if ($Method -eq 'GET' -and $Path -eq '/dashboard/stats') {
        if (-not (Test-ViewerAccessRequirement -Request $Request -Response $Response -Config $Config)) { return $true }
        $range = $Request.QueryString['range'] # today, 24h, 7d, 30d, all
        $rangeStr = if ($range) { $range.ToString().ToLower() } else { '' }
...
WHERE COALESCE(is_deleted, false) = false
  AND (
    ...
    (@range = '30d' AND completed_at >= NOW() - INTERVAL '30 days') OR
    (@range NOT IN ('today', '24h', '7d', '30d'))
  );
"@
            $row = Invoke-PGQuery -Query $scanQuery -Parameters @{ range = $rangeStr } | Select-Object -First 1
            if ($row) {
...
WHERE (
  ...
  (@range = '30d' AND lastchecked >= NOW() - INTERVAL '30 days') OR
  (@range NOT IN ('today', '24h', '7d', '30d'))
);
"@
            $compRow = Invoke-PGQuery -Query $complianceQuery -Parameters @{ range = $rangeStr } | Select-Object -First 1
            if ($compRow) {
...
function Invoke-ComplianceRoutes {
    param(
        [System.Net.HttpListenerRequest]$Request,
        [System.Net.HttpListenerResponse]$Response,
        [string]$Method,
    ...
    if ($Method -eq 'GET' -and $Path -eq '/compliance/compliant') {
        if (-not (Test-ViewerAccessRequirement -Request $Request -Response $Response -Config $Config)) { return $true }
    ...
    if ($Method -eq 'GET' -and $Path -eq '/compliance/partial') {
        if (-not (Test-ViewerAccessRequirement -Request $Request -Response $Response -Config $Config)) { return $true }
    ...
    if ($Method -eq 'GET' -and $Path -eq '/compliance/report') {
        if (-not (Test-ViewerAccessRequirement -Request $Request -Response $Response -Config $Config)) { return $true }
        try {
...
"@
    ...
    if ($Method -eq 'GET' -and $Path -eq '/compliance/history') {
        if (-not (Test-ViewerAccessRequirement -Request $Request -Response $Response -Config $Config)) { return $true }
        try {
...
function Invoke-InventoryRoutes {
    ...
    $handled = Invoke-ComputerRoutes -Request $Request -Response $Response -Method $Method -Path $Path -Config $Config
    if ($handled) { return $true }

    $handled = Invoke-ResultRoutes -Request $Request -Response $Response -Method $Method -Path $Path -Config $Config
    if ($handled) { return $true }

    $handled = Invoke-DashboardRoutes -Request $Request -Response $Response -Method $Method -Path $Path -Config $Config
    if ($handled) { return $true }

    $handled = Invoke-ComplianceRoutes -Request $Request -Response $Response -Method $Method -Path $Path -Config $Config
    if ($handled) { return $true }
